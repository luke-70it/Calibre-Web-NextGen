...
test('desktop swaps the catalog rail for fixed admin navigation and restores it on exit', async ({ page }, testInfo) => {
  ...
  await expect(adminNav.getByRole('link', { name: 'User administration' })).toHaveAttribute('aria-current', 'page');
  ...
  await expect(adminNav.getByRole('link', { name: 'Email (SMTP) server' })).toHaveAttribute('aria-current', 'page');
  ...
  await adminNav.getByRole('link', { name: 'Device administration' }).click();
  await expect(page).toHaveURL(/\/app\/admin\/devices$/);
  await expect(adminNav.getByRole('link', { name: 'Device administration' })).toHaveAttribute('aria-current', 'page');
  await expect(page.locator('main')).toHaveCount(1);
