...
def merge_kepub_metadata(tree, package):
    """Merge library metadata into an EPUB3 KEPUB package document.

    The regular metadata-backup package is an OPF2 document. Replacing a
    KEPUB's complete metadata element with it discards EPUB3-only metadata,
    including the collection form Kobo firmware uses for series. Keep the
    package document's unowned EPUB3 children and replace only the fields
    authored by :func:`create_new_metadata_backup`.
    """
    ...
    for child in generated:
        ...
        # create_new_metadata_backup builds OPF2-style, unqualified meta tags.
        # A KEPUB package is EPUB3, so keep inserted meta elements in its OPF
        # namespace rather than relying on serializer namespace inheritance.
    ...
    # EPUB3 refinements point at metadata ids. Carry those ids to the new
    # library-authored DC elements so title/creator refinements and the
    # package's unique-identifier link remain valid after the replacement.
    referenced_ids = {
    ...
    if series_name:
        ...
        collection = etree.Element(OPF + "meta", property="belongs-to-collection", id=series_id)
        collection.text = series_name
        collection_type = etree.Element(
            OPF + "meta", refines=f"#{series_id}", property="collection-type"
        )
        ...
        if series_index not in (None, ""):
            group_position = etree.Element(
                OPF + "meta", refines=f"#{series_id}", property="group-position"
            )
