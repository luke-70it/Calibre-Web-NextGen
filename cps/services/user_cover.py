...
def cover_filename(book_id: int) -> str:
    return "{}.jpg".format(int(book_id))


def cover_path(user_id: int, book_id: int) -> str:
    return os.path.join(cover_directory(user_id), cover_filename(book_id))


def version_token(row) -> str:
...
def override_for_user(user_id, book_id, *, session=None):
    ...
    if row is None or not os.path.isfile(cover_path(user_id, book_id)):
        return None
    return row
...
def overrides_for_user(user_id, book_ids, *, session=None):
    ...
    return {
        ...
        if os.path.isfile(cover_path(user_id, row.book_id))
    }
...
def stage_upload(user_id: int, book_id: int, file_storage):
    from .. import helper
    ...
    return helper.save_cover_from_filestorage(
        cover_directory(user_id), cover_filename(book_id), normalized)


def stage_bytes(user_id: int, book_id: int, raw: bytes):
    return stage_upload(
        user_id, book_id,
...
def stage_url(user_id: int, book_id: int, url: str):
    from .. import helper
...
def send_override(row):
    ...
    response = send_from_directory(
        cover_directory(row.user_id), cover_filename(row.book_id),
        mimetype="image/jpeg",
    )
...
def remove_file(user_id: int, book_id: int) -> None:
    try:
        os.remove(cover_path(user_id, book_id))
        ...
        log.warning("Could not remove cleared personal cover: %s", error)
...
def materialize_delivery_copy(user_id, book_id, source_path: str, book_format: str):
    ...
    try:
        ...
        replacement = _encode_for_member(cover_path(user_id, book_id), extension)
